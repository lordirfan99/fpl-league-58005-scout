import type { AutopilotPlayer } from "@/lib/autopilot";
import type { Bootstrap, Pick, Position } from "@/lib/types";
import { PlayerCard } from "./player-card";

const order: Position[] = ["GKP", "DEF", "MID", "FWD"];

function positionOf(player: AutopilotPlayer): Position {
  const value = player.position ?? player.pos;
  return value === "GKP" || value === "DEF" || value === "FWD" ? value : "MID";
}

function toPick(player: AutopilotPlayer, index: number, captainId?: number, viceId?: number): Pick {
...
function projected(player: AutopilotPlayer) {
  return typeof player.xpts === "number" ? `${player.xpts.toFixed(1)} xPts` : "xPts —";
}

export function ProjectionPitch({ lineup, bench, captainId, viceId, bootstrap }: { lineup: AutopilotPlayer[]; bench: AutopilotPlayer[]; captainId?: number; viceId?: number; bootstrap: Bootstrap }) {
