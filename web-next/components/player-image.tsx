...
export function PlayerImage({ photo, badgeCode, name }: { photo?: string; badgeCode?: number; name: string }) {
  ...
  if (playerUrl && !photoFailed) return <Image src={playerUrl} alt="" fill sizes="84px" onError={() => setPhotoFailed(true)} />;
  if (shirtUrl && !shirtFailed) return <span className="shirt-fallback"><Image src={shirtUrl} alt="" width={66} height={84} onError={() => setShirtFailed(true)} /></span>;
  if (badgeUrl) return <span className="badge-fallback"><Image src={badgeUrl} alt="" width={46} height={46} /></span>;
