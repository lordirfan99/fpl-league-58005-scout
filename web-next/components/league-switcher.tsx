...
export function LeagueSwitcher({ selected, pathname }: { selected: number; pathname: string }) {
  return <div className="league-switcher" aria-label="Select league">{leagues.map((league) => <Link key={league.id} href={`${pathname}?league=${league.id}`} className={selected === league.id ? "active" : ""}><span>{league.short}</span><small>{league.id.toLocaleString()}</small></Link>)}</div>;
}
