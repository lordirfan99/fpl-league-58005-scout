...
export default async function PlannerPage() {
  const autopilot = await getAutopilotData();
  const targetGameweek = autopilot?.plan?.gw;
  const data = await getPlannerData(targetGameweek);
  const plan = targetGameweek === data.gameweek ? await getCompetitiveRecommendation(data.leagueId, data.gameweek) : null;
  ...
  return <div className="page-stack"><PageHeader eyebrow={`TRANSFER PLANNER · START GW${data.fromGameweek}`} title="Plan the next five gameweeks" description={`Official FPL fixtures and difficulty for GW${data.fromGameweek}–GW${data.toGameweek}. This begins at the same upcoming deadline as the decision board.`} />
    <section className="surface planner-note"><span>CANONICAL STARTING POINT</span><h2>{autopilot?.plan?.decision_summary?.recommended_action === "LINEUP ONLY" ? "Hold the transfer for now" : autopilot?.plan?.decision_summary?.recommended_action ?? "Decision pending"}</h2><p>{autopilot?.plan?.decision_summary?.reason ?? "A fresh planning decision has not been published. Use the fixture outlook for research only."}</p></section>
