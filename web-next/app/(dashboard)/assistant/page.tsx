...
export default async function AssistantPage() {
  ...
  const { manager } = data;
  const elite = buildRecommendations(manager, data.managers, data.bootstrap, data.fixture, data.gameweek);
  ...
  return <div className="page-stack">
    <PageHeader eyebrow="DECISION ASSISTANT" title="Your weekly decision board" description="Catch the elite baseline first, then preserve it and attack only with model-supported edges." updated={botPlan?.generated_at ? new Date(botPlan.generated_at).toLocaleString("en-MY", { dateStyle: "medium", timeStyle: "short" }) : data.fetchedAt ? new Date(data.fetchedAt).toLocaleString("en-MY", { dateStyle: "medium", timeStyle: "short" }) : undefined} />
