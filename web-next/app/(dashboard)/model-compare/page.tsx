...
function ModelXi({ label, version, mode, lineup, bench, captainId, viceId, points, robustPoints, unique, uniqueLabel = "UNIQUE VS LIVE XI", comparisonAvailable = true, transferNote, bootstrap }: { label: string; version: string; mode: "live" | "lab"; lineup: LineupPlayer[]; bench: LineupPlayer[]; captainId?: number; viceId?: number; points: number; robustPoints?: number; unique: string[]; uniqueLabel?: string; comparisonAvailable?: boolean; transferNote?: string; bootstrap: Bootstrap }) {
  return <section className={`surface model-xi ${mode}`}>
    ...
    <div className="model-xi-evidence"><div><span>{uniqueLabel}</span><p>{comparisonAvailable ? unique.join(", ") || "None — the compared XIs match." : "N/A — a same-gameweek comparison is not available."}</p></div><div><span>TRANSFERS</span><p>{transferNote ?? "Not modelled — the V5 lab optimises the XI from your live 15 only."}</p></div></div>
  </section>;
}

export default async function ModelComparePage() {
  const [dashboard, live, v5] = await Promise.all([getDashboardData(), getLiveTeam(), getV5Projections()]);
  const upcoming = dashboard.bootstrap.events.find((event) => !event.finished);
  const targetGameweek = upcoming?.id ?? dashboard.gameweek + 1;
  ...
  const toLineup = (pick: (typeof livePicks)[number]): LineupPlayer => {
    const meta = dashboard.bootstrap.elements.find((row) => row.id === pick.element);
  ...
  const v5Comparable = v5.available && v5.gameweek === targetGameweek;
  ...
  const overlap = v5Selection ? [...liveIds].filter((id) => new Set(v5Selection.lineup.flatMap((player) => player.id == null ? [] : [player.id])).has(id)).length : 0;

  return <div className="page-stack model-compare-page">
    <PageHeader eyebrow={`MODEL XI LAB · GW${targetGameweek}`} title="Compare model team selections" description="The same player images and formation view as My Team, comparing your official live XI with the V5 lineup lab's pick from the same 15. Read-only — every change is made manually in FPL." />
    <section className="metric-grid"><MetricCard label="Live XI xPts" value={liveReady ? livePoints.toFixed(1) : "—"} detail="Your current 15" tone="positive" /><MetricCard label="V5 lineup lab" value={v5Ready ? formation(v5Selection?.lineup ?? []) : "Unavailable"} detail={v5.projection_version} tone={v5Ready ? "warning" : undefined} /><MetricCard label="XI overlap" value={v5Ready ? `${overlap}/11` : "—"} detail="Live versus V5 lab" /><MetricCard label="Target gameweek" value={`GW${targetGameweek}`} detail={v5Comparable ? "V5 aligned" : "V5 targets another GW"} /></section>
    {liveReady ? <div className="model-compare-grid">
      <ModelXi label="YOUR LIVE XI" version={`GW${live?.gameweek ?? targetGameweek} squad`} mode="live" lineup={liveLineup} bench={liveBench} captainId={liveCaptain} viceId={liveVice} points={livePoints} unique={v5Ready ? namesOnly(liveLineup, new Set(v5Selection!.lineup.flatMap((player) => player.id == null ? [] : [player.id]))) : []} uniqueLabel="UNIQUE VS V5 LAB XI" comparisonAvailable={v5Ready} transferNote="Your real squad — no model transfers applied." bootstrap={dashboard.bootstrap} />
      {v5Ready && v5Selection ? <ModelXi label="V5 LINEUP LAB" version={v5.projection_version} mode="lab" lineup={v5Selection.lineup} bench={v5Selection.bench} captainId={v5Selection.captainId} points={v5Selection.points} robustPoints={v5Selection.robustPoints} unique={namesOnly(v5Selection.lineup, liveIds)} bootstrap={dashboard.bootstrap} /> : <section className="surface model-xi-empty"><FlaskConical /><h2>No comparable V5 lineup</h2><p>{v5.available && v5.gameweek !== targetGameweek ? `V5 targets GW${v5.gameweek}; the next deadline is GW${targetGameweek}. Cross-gameweek comparison is blocked.` : v5.reason ?? "V5 does not have projections for all 15 of your live-squad players."}</p></section>}
    ...
    <p className="execution-note"><strong>Interpretation:</strong> a different XI shows a model disagreement, not proof that one XI will score more. The V5 lab is research only; it is never an instruction and never writes to FPL.</p>
  </div>;
}
