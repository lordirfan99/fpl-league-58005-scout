...
function ModelXi({ label, version, mode, lineup, bench, captainId, viceId, points, robustPoints, unique, transfers, transferNote, bootstrap }: { label: string; version: string; mode: "champion" | "shadow" | "lab"; lineup: AutopilotPlayer[]; bench: AutopilotPlayer[]; captainId?: number; viceId?: number; points: number; robustPoints?: number; unique: string[]; transfers?: ShadowTransfer[]; transferNote?: string; bootstrap: Bootstrap }) {
  return <section className={`surface model-xi ${mode}`}>
    ...
    <div className="model-xi-evidence"><div><span>UNIQUE VS PRODUCTION XI</span><p>{unique.join(", ") || "None — this model selected the production XI."}</p></div><div><span>FIRST-GW TRANSFERS</span><p>{transferNote ?? (transfers?.length ? transfers.map((move) => `${move.out_name ?? "?"} → ${move.in_name ?? "?"}`).join(" · ") : "None")}</p></div></div>
  </section>;
}
