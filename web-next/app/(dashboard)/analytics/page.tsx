...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<{ league?: string }> }) {
  ...
  return <div className="page-stack"><PageHeader eyebrow="LEAGUE ANALYTICS" title={`${selected.name} data lab`} description="Formation, points, value, captaincy and elite-vs-league ownership patterns from the live snapshot." updated={data.fetchedAt ? new Date(data.fetchedAt).toLocaleString("en-MY", { dateStyle: "medium", timeStyle: "short" }) : undefined} /><LeagueSwitcher selected={selected.id} pathname="/analytics" />
