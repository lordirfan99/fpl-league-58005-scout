...
export default async function TransfersPage({ searchParams }: { searchParams: Promise<{ league?: string }> }) {
  ...
  const optimizer = await getTransferOptimizer(selected.id, data.gameweek);
  return <div className="page-stack"><PageHeader eyebrow="MARKET INTELLIGENCE" title="Transfers & chips" description="Track what the complete league and its top-5% cohort actually did in the selected gameweek." updated={data.fetchedAt ? new Date(data.fetchedAt).toLocaleString("en-MY", { dateStyle: "medium", timeStyle: "short" }) : undefined} /><LeagueSwitcher selected={selected.id} pathname="/transfers" />
    ...
    <section className="surface table-surface"><div className="section-heading"><div><span>NET-EV RESEARCH · {optimizer.optimizer_version}</span><h2>Multi-week legal transfer plans</h2><p>Includes hit cost, budget, club limits, uncertainty penalty and the future value of a saved free transfer. Read-only.</p></div><span className="section-chip">GW{optimizer.target_gameweeks.join("–GW")}</span></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Plan</th><th>Moves</th><th>Gross horizon</th><th>Hits</th><th>FT value</th><th>NET EV</th><th>Bank</th></tr></thead><tbody>{optimizer.plans.slice(0, 6).map((plan, index) => <tr key={`${plan.transfer_count}-${index}`}><td><strong>{plan.transfer_count ? `Option ${index + 1}` : "Hold"}</strong></td><td>{plan.transfers?.map((move) => `${move.out_name} → ${move.in_name}`).join("; ") || "Roll transfer"}</td><td>{(plan.gross_horizon_gain ?? 0).toFixed(2)}</td><td>-{plan.hit_cost.toFixed(1)}</td><td>-{(plan.free_transfer_opportunity_cost ?? 0).toFixed(1)}</td><td><strong>{(plan.net_ev ?? 0).toFixed(2)}</strong></td><td>£{plan.bank_after.toFixed(1)}m</td></tr>)}</tbody></table></div><p className="research-note">{optimizer.disclaimer}</p></section>
