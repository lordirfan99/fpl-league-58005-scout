...
The external production run additionally found Netlify's injected badge intercepting the mobile `More` control, followed by an early-tap hydration race. The control stays disabled until React is ready, and the final `017dd0c` UI hides the injected badge below 720 px so it neither intercepts nor visually covers navigation. The current regression suite asserts enabled/expanded navigation state, visits all 17 dashboard routes, checks phone-width overflow, verifies numeric live-team values and exercises the public-league Transfers fallback. The final local run passed 26 tests with two intentional project-specific skips; the deployed 17-route desktop/mobile audit also passed. No known P0/P1 frontend defect remains; the frontend release gate is **PASS**, with manual assistive-technology breadth retained as follow-up work.
